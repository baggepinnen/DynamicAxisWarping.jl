...
# Visualize the coupling matrix. The green line should cross diagonally (since the two signals are the same) starting halfway along the axes (since one signal is shiften by 50% of the length copared to the other).
...
n = size(M1.power, 1)
dist = DiscreteGridTransportDistance(SqEuclidean(), zeros(n,n))
...
# We can create the need for som transport along the frequency axis
